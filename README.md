...
This project submission needs only to solve one of the two versions of the environment. 
...
```
(base) ➜  drlnd-p2 git:(master) ✗ conda create --name drlnd-p2 python=3.6
(base) ➜  drlnd-p2 git:(master) ✗ conda activate drlnd-p2
```
...
Start a `jupyter notebook` from within the project folder and follow the instructions in `notebooks/Continuous_Control.ipynb` to either
